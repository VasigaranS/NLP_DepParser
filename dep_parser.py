...
class Parser: 
    ...
    def move(self, i, stack, pred_tree, move):
        ...
        if(move==self.SH ):
            ...
            pred_tree[stack[l-2]]=stack[l-1]
            stack.pop(l-2)

        elif(move==self.RA):
            pred_tree[stack[l-1]]=stack[l-2]
            stack.pop(l-1)
    ...
    def compute_correct_move(self, i,stack,pred_tree,correct_tree) :
        ...
        try:
            ...
            l=len(stack)                        
            # YOUR CODE HERE
            if( i == len(correct_tree) and stack == [0] ):
                return None


            

            elif(correct_tree[stack[l-2]] ==stack[l-1] ):
                if(i<len(correct_tree)):
                    if(stack[l-2] not in correct_tree[i:]):
                        return self.LA
                else:
                    return self.LA

            elif(correct_tree[stack[l-1]]==stack[l-2] ):


                if(i<len(correct_tree)):
                    if(stack[l-1] not in correct_tree[i:]):
                        return self.RA
                else:
                    return self.RA
